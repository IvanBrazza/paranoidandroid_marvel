...
#include <rpc/rpc.h> include <arpa/inet.h> include 
#<rpc/rpc_router_ioctl.h> include <debug.h> include <pthread.h> include 
#<sys/select.h> include <stdarg.h> include <stdio.h> include <string.h> 
#include <errno.h> include <stdlib.h> include <hardware_legacy/power.h> 
#include <poll.h> define ANDROID_WAKE_LOCK_NAME "rpc-interface"
...
struct CLIENT {
    ...
    /* We insist that there is only one outstanding call for a client at 
any
       ...
       maintain a queue of them, and match incoming replies (by the XID 
of the
       incoming packet).  For now, we just block until we get that 
reply.
    */
...
extern void svc_dispatch(void *svc, void *xprt); extern int r_open(const 
char *name); extern void r_close(int handle); extern xdr_s_type 
*xdr_init_common(const char *name, int is_client); extern xdr_s_type 
*xdr_clone(xdr_s_type *); extern void xdr_destroy_common(xdr_s_type 
*xdr); extern bool_t xdr_recv_reply_header (xdr_s_type *xdr, 
rpc_reply_header *reply); extern void *the_xprt; extern int 
svc_is_in_reset(void* xprt); extern void svc_set_in_reset(void* xprt, 
int val); extern void svc_reset_cb(void* xprt, enum rpc_reset_event 
event); static pthread_mutex_t rx_mutex = PTHREAD_MUTEX_INITIALIZER; 
static pthread_t rx_thread; static volatile unsigned int num_clients; 
static volatile struct CLIENT *clients; static int router_fd; /* pipe 
used to unblock receive thread using self-pipe method */ static int 
wakeup_pipe[2]; /* There's one of these for each RPC client which has 
received an RPC call. */ static void *cb_context(void *__u) {
    ...
    while (client->cb_stop == 0) {
        ...
        /* We tell the thread it's time to exit by setting cb_stop to 
nonzero
           and signalling the conditional variable.  When there's no 
data, we
           skip to the top of the loop and exit.
        */
        if (!client->got_cb) {
            LIBRPC_DEBUG("RPC-callback thread for %08x:%08x: signalled 
but no data.\n",
              (client->xdr->x_prog | 0x01000000),
              client->xdr->x_vers);
            continue;
        ...
        client->got_cb = 0;
        /* We dispatch the message to the server representing the 
callback
         * client.
         */
        if (the_xprt) {
            ...
            if (svc) {
                XDR **svc_xdr = (XDR **)svc;
                LIBRPC_DEBUG("%08x:%08x dispatching RPC call (XID %d, 
xdr %p) for "
                ...
                /* We transplant the xdr of the client into the entry
                   representing the callback client in the list of 
servers.
                   Note that since we hold the wait_cb_lock for this 
client,
                   if another call for this callback client arrives 
before
                   we've finished processing this call, that will block 
until
                   we're done with this one.  If this happens, it would 
be
...
static void *rx_context(void *__u __attribute__((unused))) {
    ...
    while(num_clients) {
        ...
        if (pbits == NULL || num_clients_cached != num_clients) {
            ...
            pbits = calloc(num_clients_cached + 1, sizeof(struct 
pollfd));
            /* add wakeup pipe */
        ...
        for (n=1; client; client = client->next, n++) {
            ...
            if (!client->in_reset) {
                if (pbits[n].revents & POLLRDHUP) {
                    LIBRPC_DEBUG("modem entered reset for client %p, 
cb=%p\n",
...
RPC_SUBSYSTEM_RESTART_BEGIN);
                ...
                LIBRPC_DEBUG("modem exited reset for client %p, 
cb=%p\n",
                        client, client->reset_cb);
                /* clear reset flags */
                pthread_mutex_lock(&client->wait_reply_lock);
...
bool_t xdr_recv_auth (xdr_s_type *xdr, opaque_auth *auth) {
    ...
    case 1:
        if(!XDR_RECV_INT8(xdr, (int8_t *)&(auth->oa_flavor))) return 
FALSE;
        break;
    case 2:
        if(!XDR_RECV_INT16(xdr, (int16_t *)&(auth->oa_flavor))) return 
FALSE;
        break;
    case 4:
        if(!XDR_RECV_INT32(xdr, (int32_t *)&(auth->oa_flavor))) return 
FALSE;
        break;
    }
...
static bool_t xdr_recv_accepted_reply_header(xdr_s_type *xdr,
...
CLIENT *clnt_create(
    ...
    if (client) {
        char name[20];
        /* for versions like 0x00010001, only compare against major 
version */
	...
	if (!num_clients) {
        ...
        vers = (vers & 0x80000000 || vers == 0x20002) ? vers : vers & 
0xFFFF0000;
        snprintf(name, sizeof(name), "%08x:%08x", (uint32_t)prog, 
(int)vers);
...
pthread_mutexattr_settype(&client->lock_attr, PTHREAD_MUTEX_RECURSIVE);
        ...
        pthread_mutex_init(&client->wait_reply_lock, 
&client->lock_attr);
...
int clnt_register_reset_notification_cb(CLIENT *client, 
clnt_reset_notif_cb cb) {
    int ret = 1;
    if (client) {
        pthread_mutex_lock(&client->lock);
        client->reset_cb = cb;
...
clnt_reset_notif_cb clnt_unregister_reset_notification_cb(CLIENT 
*client) {
...
*/
        ...
        pthread_mutex_unlock(&rx_mutex); /* sync access to the client 
list */
        ...
        // FIXME: what happens when we lock the client while destroying 
it,
        // and another thread locks the mutex in clnt_call, and then we
        // call pthread_mutex_destroy?  Does destroy automatically 
unlock and
        // then cause the lock in clnt_call() to return an error?  When 
we
        // unlock the mutex here there can be a context switch to the 
other
        // thread, which will cause it to obtain the mutex on the 
destroyed
        // client (and probably crash), and then we get to the destroy 
call
